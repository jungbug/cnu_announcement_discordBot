...
async def fetch_cnu_announcement():
    async with aiohttp.ClientSession() as session:
        ...
        return title.attrs['title'], f"{cnu_front_url+title.attrs['href']}#list"

async def send_announcement_to_channel(channel, announcement, link):
...
async def my_function():
    ...
    if new_value != previous_value:
        announcement, link = new_value
        channel_id = int(discord_channel_id)
        channel = bot.get_channel(channel_id)
        if channel:
            await send_announcement_to_channel(channel, announcement, link)
